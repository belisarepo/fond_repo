		...
		<tbody>
			<c:forEach items="${zayavkaModel.calcZpSet}" var="i">
				<tr>
					...
					<td>${i.note}</td>
					<td><a
						href="<portlet:actionURL><portlet:param name="action" value="deleteCalcZp"/><portlet:param name="calcZpId" value="${i.id}"/><portlet:param name="konkursId" value="${zayavkaModel.konkursId}" /><portlet:param name="zayavkaId" value="${zayavkaModel.id}" /></portlet:actionURL>">Удалить</a></td>
				</tr>
			</c:forEach>
			<tr>
