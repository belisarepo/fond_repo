...
<div id="publRuk">
	...
	<c:if test="${not empty zayavkaModel.fizInfoDTO.publications}">
		<table class="table table-condensed table-bordered" style="width: auto">
			...
			<tbody>
				<c:forEach items="${zayavkaModel.fizInfoDTO.publications}" var="i">
					<tr>
						<td>${i.pn}</td>
	...
	<aui:form action="${addPublicationMAction}" method="POST" name="form20/1">
		...
		<aui:input name="name" label='${publNameLabel}' bean="publicationMModel">
			<aui:validator name="required" />
		</aui:input>

		<spring:message code="zayavka.publIzdanie" var='publIzdanieLabel' />
		<aui:input name="edition" label='${publIzdanieLabel}' bean="publicationMModel">
			<aui:validator name="required" />
		</aui:input>

		<spring:message code="zayavka.pages" var='pagesLabel' />
		<aui:input name="pages" label='${pagesLabel}' bean="publicationMModel">
			<aui:validator name="required" />
		</aui:input>

		<spring:message code="zayavka.soauthors" var='soauthorsLabel' />
		<aui:input name="authors" label='${soauthorsLabel}' bean="publicationMModel">
			<aui:validator name="required" />
		</aui:input>
	...
	<div id="publIsp${n}">
		...
		<c:if test="${not empty ispl.fizInfo.publications}">
			<table class="table table-condensed table-bordered" style="width: auto">
				...
				<tbody>
					<c:forEach items="${ispl.fizInfo.publications}" var="i">
						<tr>
							<td>${i.pn}</td>
							<td>${i.publicationTypeName}</td>
							<td>${i.name}</td>
