		...
		<tbody>
			<c:forEach items="${zayavkaModel.petitionSet}" var="i" varStatus="n">
				<tr>
					...
					<td>${i.name}</td>
					<td><a
						href="<portlet:actionURL><portlet:param name="action" value="deletePetition"/><portlet:param name="petitionId" value="${i.id}"/><portlet:param name="konkursId" value="${zayavkaModel.konkursId}" /><portlet:param name="zayavkaId" value="${zayavkaModel.id}" /></portlet:actionURL>">Удалить</a></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
